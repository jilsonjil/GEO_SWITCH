        ...
        FirebaseDatabase.getInstance().getReference("user").child(uname)
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot postSnapshot: snapshot.getChildren()) {
                    ...
                    try {
                        if (format.parse(timemobileprofilestores.getDate()).getTime() >= (System.currentTimeMillis() - (
                                1000 * 60 *  60 * 24
                        )) )
                ...
                if(!data.isEmpty()) {
                    ...
                    data.sort(new Comparator<timemobileprofilestore>() {
                        @Override
                        public int compare(timemobileprofilestore o1, timemobileprofilestore o2) {
                            SimpleDateFormat format = new SimpleDateFormat("MMM dd, yyyy");
                            try {
